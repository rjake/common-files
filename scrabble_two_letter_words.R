...
starting_summary %>% 
  ...
  geom_tile(aes(fill = log(pct*1000)), color = "grey40") +
  geom_text(
    aes(label = count), color = "white"
  ) +
  ...
  labs(
    ...
    subtitle = paste0('This chart shows how many paths will allow you to keep making words by adding one letter on each turn. 
With "at" as an example, there are two ways to get a nine letter word:
at -> eat -> eath/heat -> heath -> sheath -> sheathe -> sheather -> sheathers'),
    x = "word length", y = "two letter words"
  )
