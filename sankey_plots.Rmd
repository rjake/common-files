    ...
    df_set <-
        ...
        #line up y-positions for col_1
            ...
            mutate(add_gap_1 = (col_1 == lag(col_1, default = first(col_1))),
                   y1 = cumsum(N + ifelse(add_gap_1, 0, gap))) %>% 
            ungroup() %>% 
        #repeat for col_2
            arrange(n_col_2, N) %>% 
            mutate(add_gap_2 = (col_2 == lag(col_2, default = first(col_2))),
                   y2 = cumsum(N + ifelse(add_gap_2, 0, gap)))
...
ggplot(a) +
    geom_ribbon(aes(x = x, ymax = high, ymin = low,
                    group = group, fill = col_1), alpha = 0.25) +
    geom_rect(data = filter(a, x %in% c(min(x), max(x))),
        aes(xmin = x - 0.1, xmax = x + 0.1, 
            ymin = low, ymax = high+0.5,
            group = group), fill = "black") +
    guides(fill = F, color = F) +
    geom_text(
        data = a %>% group_by(col_1) %>% filter(x == 1) %>% 
            summarise(x = 1, high = max(high)),
        aes(x = x, y = high, label = col_1),
        hjust = 1, nudge_x = -0.2
        ) +
    geom_text(
        data = a %>% group_by(col_2) %>% filter(x == 11) %>% 
            summarise(x = 11, high = max(high)),
        aes(x = x, y = high, label = col_2),
        hjust = 0, nudge_x = 0.2
        ) +
